...
{
    public partial class frmAca_Nota_Deb_Mant : Form
    {
        ...
        public void GetInfo()
        {
            ...
            try
            {
                ...
                Info_Nota_Deb.Info_notaCredDeb_aca = Get_Info_notaCredDeb_aca();
            }
            catch (Exception ex)
            {
                string NameMetodo = System.Reflection.MethodBase.GetCurrentMethod().Name;
        ...
        private fa_notaCredDeb_aca_Info Get_Info_notaCredDeb_aca()
        {
            try
            {
                ...
                InfoNotaCredDeb_aca.IdBodega = Info_Nota_Deb.IdBodega;
                InfoNotaCredDeb_aca.IdEmpresa = Info_Nota_Deb.IdEmpresa;
                InfoNotaCredDeb_aca.IdSucursal = Info_Nota_Deb.IdSucursal;
                InfoNotaCredDeb_aca.IdInstitucion = param.IdInstitucion;
                InfoNotaCredDeb_aca.IdEstudiante = ucAca_Estudiante1.Get_Estudiante().IdEstudiante;
                InfoNotaCredDeb_aca.Observaciones = txtObservacion.Text;
                InfoNotaCredDeb_aca.IdNotaCredDeb = Info_Nota_Deb.IdNota;
        ...
        private void Set_Nota_Debito()
        {
            try
            {
                ...
                ucAca_Estudiante1.set_Estudiante(InfoNotaCredDeb_aca.IdEstudiante);
        ...
        private void SetInfo_fa_notaCredDeb_aca()
        {
            try
            {
                InfoNotaCredDeb_aca = BusNotaCredDeb_aca.Get_Info(Info_Nota_Deb.IdEmpresa, Info_Nota_Deb.IdSucursal, Info_Nota_Deb.IdBodega, Info_Nota_Deb.IdNota);
                    ...
                    if (MessageBox.Show("La " + ctrl_NumDoc.IdTipoDocumento + " # " + talonarioInfo.Establecimiento + "-" + talonarioInfo.PuntoEmision + "-" + talonarioInfo.NumDocumento + " se procedera a Guardar." + "\n" + "¿Desea Continuar?", "Guardar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        if (Bus_notaCreDeb.VerificarCodigo(txtCodigoNot.Text) == false)
                        {
                            ...
                            if (Bus_notaCreDeb.GuardarDB(Info_Nota_Deb, ref Idnota,  ref mensajeDocumentoDupli, ref mensajeError))
                            {
                                ...
                                Info_Nota_Deb.Info_notaCredDeb_aca.IdNotaCredDeb = Info_Nota_Deb.IdNota;
                                if (BusNotaCredDeb_aca.GrabarDB(Info_Nota_Deb.Info_notaCredDeb_aca, ref mensajeError))
                                {
                                    bolResult = true;
            ...
            try
            {
                ...
                if (Validar())
                {
                    ...
                    if (Bus_notaCreDeb.ActualizarDB(Info_Nota_Deb, ref mensajeError, ref mensajeDocumentoDupli))
                    {
                        ...
                        if (BusNotaCredDeb_aca.ActualizarDB(Info_Nota_Deb.Info_notaCredDeb_aca, ref mensajeError))
                        {
                            bolResult = true;
        ...
        void LimpiarDatos()
        {
            try
            {
                ...
                Info_Nota_Deb.Info_notaCredDeb_aca = new fa_notaCredDeb_aca_Info();
                _Accion = Cl_Enumeradores.eTipo_action.grabar;
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                else
                {
                    foreach (var item in Info_Nota_Deb.ListaDetalles)
                    {
                        ...
                        //{
                        //    if(Info_Doc_rel.Valor_Aplicado 
                        //}
                    }
                }
                if (Info_Nota_Deb.Info_notaCredDeb_aca.IdEstudiante == 0)
                {
                    MessageBox.Show(param.Get_Mensaje_sys(enum_Mensajes_sys.Por_Favor_) + " " + param.Get_Mensaje_sys(enum_Mensajes_sys.Seleccione_el) + " Estudiante", param.Nombre_sistema, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
