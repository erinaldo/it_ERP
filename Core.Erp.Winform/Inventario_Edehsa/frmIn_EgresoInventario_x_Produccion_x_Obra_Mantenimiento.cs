...
{
    public partial class frmIn_EgresoInventario_x_Produccion_x_Obra_Mantenimiento : Form
    {
        #region variables
        ...
        public delegate void delegate_frmIn_EgresoInventario_x_Produccion_x_Obra_Mantenimiento_FormClosing(object sender, FormClosingEventArgs e);
        public event delegate_frmIn_EgresoInventario_x_Produccion_x_Obra_Mantenimiento_FormClosing event_frmIn_EgresoInventario_x_Produccion_x_Obra_Mantenimiento_FormClosing;
        string mensaje = "";
        #endregion

        public frmIn_EgresoInventario_x_Produccion_x_Obra_Mantenimiento()
        {
            try
            {
                InitializeComponent();
                ...
                this.event_frmIn_EgresoInventario_x_Produccion_x_Obra_Mantenimiento_FormClosing +=
                    frmIn_EgresoInventario_x_Produccion_x_Obra_Mantenimiento_event_frmIn_EgresoInventario_x_Produccion_x_Obra_Mantenimiento_FormClosing;
        ...
        void cargadisponible()
        {
            try
            {
                if (iAccion == Cl_Enumeradores.eTipo_action.grabar)
                {
                    ...
                    //ListadoProductos = BusDetxItems.SaldosxItemsxCodBarra_x_Obra_x_OT(param.IdEmpresa, dtpFecha.Value, ctrl_Sucbod.get_bodega().IdBodega, ctrl_Sucbod.get_sucursal().IdSucursal);

                    //Se cambio debido a que ya no se puede registrar un listado de compra por Obra
                    //ListadoProductos = BusDetxItems.SaldosxItemsxCodBarra_x_Obra_x_OT(param.IdEmpresa, dtpFecha.Value, ctrl_Sucbod.get_sucursal().IdSucursal, ctrl_Sucbod.get_bodega().IdBodega, UCObra.get_item(), Convert.ToInt32(CmbOrdenTaller.EditValue));
        ...
        private void frmIn_EgresoInventario_x_Produccion_x_Obra_Mantenimiento_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void frmIn_EgresoInventario_x_Produccion_x_Obra_Mantenimiento_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
               event_frmIn_EgresoInventario_x_Produccion_x_Obra_Mantenimiento_FormClosing(sender, e);
            }
            catch (Exception ex)
            {
                Log_Error_bus.Log_Error(ex.ToString());
        ...
        void frmIn_EgresoInventario_x_Produccion_x_Obra_Mantenimiento_event_frmIn_EgresoInventario_x_Produccion_x_Obra_Mantenimiento_FormClosing(object sender, FormClosingEventArgs e){}
     ...
     catch (Exception ex)
     {
            ...
            try
            {
                ...
                foreach (var item in LstProdGrid)
                {
                  ...
                  in_movi_inve_detalle_x_Producto_CusCider_Info ItemCodBarEgr = new in_movi_inve_detalle_x_Producto_CusCider_Info();
                    ...
                    /////////oc_IdOrdenCompra para trazabilidad
                    ItemCodBarIng.ocd_IdOrdenCompra = item.oc_IdOrdenCompra;
                    ItemCodBarIng.NumDocumentoRelacionadoProveedor = item.NumDocumentoRelacionadoProveedor;
                    ItemCodBarIng.NumFacturaProveedor = item.NumFacturaProveedor;
                  ...
                  " Ing x Cons Prod -" + MovIngreso.cm_observacion + " " + txtObservacion.Text + ing.dm_observacion;
                    ...
                    ///////// ocd_IdOrdenCompra para reporte de Trazabilidad
                    ItemCodBarEgr.ocd_IdOrdenCompra = item.oc_IdOrdenCompra;
                    ItemCodBarEgr.NumDocumentoRelacionadoProveedor = item.NumDocumentoRelacionadoProveedor;
                    ItemCodBarEgr.NumFacturaProveedor = item.NumFacturaProveedor;
        ...
        {
            // se llama al que llena el grid
            //Se cambio debido a que ya no se asocia en el listado 
            //de requerimiento a una ordne de taller sino a la obra
            //cargadisponible();
        }
