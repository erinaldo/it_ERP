...
{
    public class ct_distribucion_gastos_x_periodo_det_gastos_Bus
    {
        ct_distribucion_gastos_x_periodo_det_gastos_Data oData = new ct_distribucion_gastos_x_periodo_det_gastos_Data();
        ...
        public List<ct_distribucion_gastos_x_periodo_det_gastos_Info> get_list(int IdEmpresa, decimal IdDistribucion)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new Core.Erp.Info.Log_Exception.DalException(string.Format("", "Get_Info_Punto_cargo", ex.Message), ex) { EntityType = typeof(ct_distribucion_gastos_x_periodo_det_gastos_Bus) };
            }
        }
        public List<ct_distribucion_gastos_x_periodo_det_gastos_Info> get_list_para_distribucion(int IdEmpresa, int IdPeriodo)
        {
            try
            {
                return oData.get_list_para_distribucion(IdEmpresa, IdPeriodo);
            }
            catch (Exception ex)
            {
                Core.Erp.Info.Log_Exception.LoggingManager.Logger.Log(Core.Erp.Info.Log_Exception.LoggingCategory.Error, ex.Message);
